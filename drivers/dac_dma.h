...
static inline void dac_gpio_init(uint16_t pin, uint16_t conf){
  gpio_enable(pin); 
...
static inline void dac_ch1_normal_mode(void){
    ...
    DAC->CR &= ~BIT(14); // turn off calibration ch 1
...
static inline void dac_ch1_write_12bit(uint16_t value) {
    DAC->DHR12R1 = value & 0x0FFFu; // if Vref is 3.3, 2048 / 4095 * 3.3V ≈ 1.65V, 0x0FFFu = 4095
